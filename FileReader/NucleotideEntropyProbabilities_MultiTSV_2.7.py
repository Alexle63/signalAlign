...
def startup(startpos, rang, pos, opath, tsvfile):
    ...
    del FILE_DATA[:]
    del CHROMOSOME[:]
    del CHROMOSOME_VALUE[:]
    del CHROMOSOME_COUNT[:]
    del MINIMUM_X[:]
    del MINIMUM_Y[:]
    del MIN_EVENT_X[:]
    del MIN_EVENT_Y[:]
    ...
    del SHORT_PROBABILITIES[:]
    del NUCLEOTIDES[:]
    del highestprobarray[:]
    del totalofaverage[:]
    del pesouttheorem[:]
    del probabilities[:]
    del reciprocals[:]
    del harmonicbottom[:]
    del harmonicmeanfinal[:]
    del hentropy[:]
    del Alist[:]
    del Clist[:]
    del Glist[:]
...
def chromosomesort(my_data2, substringcounter, x_coords, subcounter, basecounter, substringposcounter, Atotal, Ctotal, Gtotal, Ttotal, wholetotal):
    ...
    for r in x_coords:
        ...
        del chromosomeprob['A'][:]
        del chromosomeprob['G'][:]
        del chromosomeprob['C'][:]
        del chromosomeprob['T'][:]
        subcounter = 0
        substringcounter+=1
